...
* If you opted to do the wireframe (easier) assignment for last homework or if you struggled with the portfolio design, you can find starter code for this assignment here: [starter-code](starter-code)
...
1. Inside your `Bootstrap-Portfolio` repo, copy the `index.html`, `portfolio.html`, `contact.html`, and the `assets` folder with the `style.css` from the [starter-code](starter-code) code folder
...
1. Inside your `Responsive-Portfolio` repo, copy the `index.html`, `portfolio.html`, `contact.html`, and the `assets` folder with the `style.css` from the [starter-code](starter-code) code folder
...
3. Use three `@media screen` tags, each with one of these `max-width`s: `980px`, `768px` and `640px`.
...
6. Be sure to include the `viewport` tag in all your HTML files, otherwise your media-queries won't function as expected on mobile devices. _(Hint: You won't need to use exact pixels for anything other than the container)_

7. **Protip**: Use the Chrome extensions [Window Resizer](https://chrome.google.com/webstore/detail/window-resizer/kkelicaakdanhinjdeammmilcgefonfh) and [Browser Width](https://chrome.google.com/webstore/detail/browser-width/mlnegepkjlccabakompdmbcmdieaideh) to see the browser dimensions in Chrome.

8. Deploy your new portfolio (now with media queries!) to GitHub Pages.

### Submitting Your Work on [BootCampSpot](https://www.bootcampspot.com/)
